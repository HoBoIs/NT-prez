...
function createWindow () {
  ...
  */
  ...
  electron.ipcMain.on('to_main_refr', (event, arg) => {
    event.sender.send('nameReply', { not_right: false })
    mainWindow.webContents.send( 'to_m_refr', arg ); 
  })
  electron.ipcMain.on('to_set_audioevent', (event, arg) => {
    event.sender.send('nameReply', { not_right: false })
  ...
  electron.ipcMain.on('to_set_flines', (event, arg) => {
    event.sender.send('nameReply', { not_right: false })
    setupWindow.webContents.send( 'to_s_flines', arg ); 
  })
  electron.ipcMain.on('to_set_content', (event, arg) => {
    event.sender.send('nameReply', { not_right: false })
    setupWindow.webContents.send( 'to_s_content', arg ); 
  })
  electron.ipcMain.on('to_main_lent', (event, arg) => {
    event.sender.send('nameReply', { not_right: false })
    mainWindow.webContents.send( 'to_m_lent', arg ); 
  })
}
...
// code. You can also put them in separate files and require them here. TEST
